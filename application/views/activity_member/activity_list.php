...
<!-- Basic Button table start -->
		...
		<?php echo anchor(site_url('activity_member/excel'), '<i class="fa fa-file-excel-o"></i> Export as Excel', 'class="btn btn-success btn-square"'); ?>
		<?php echo anchor(site_url('activity_member/word'), '<i class="fa fa-file-word-o"></i> Export as Word', 'class="btn btn-inverse btn-square"'); ?>
                </div>
        ...
        <script type="text/javascript">
            $(document).ready(function() {
                ...
                var t = $("#mytable").dataTable({
                    ...
                    processing: true,
                    serverSide: true,
                    ajax: {"url": "activity_member/json", "type": "POST"},
                    columns: [
                        {
                            "data": "act_id",
                            "orderable": false
        ...
        <?php if ($this->session->flashdata('message')) { ?>
            ...
            <?php }else if($this->session->flashdata('message') == "failed") { ?>
                 <script type="text/javascript">
                   ...
                   swal({
                        title: "ERROR !",
                        text: "You already join this activity!",
                        timer: 1600,
                        showConfirmButton: false,
                        allowOutsideClick: true,
                        type: 'error'
                 ...
                 $.ajax({
                     ...
                     success: function(data) {
                        if(true)
                          ...
                          swal("ERROR", "Waiting for approval.", "error");
                     }
                  });
                } 
